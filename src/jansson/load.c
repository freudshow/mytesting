...
static void error_set(json_error_t *error, const lex_t *lex, enum json_error_code code,
    ...
    char msg_with_context[2 * JSON_ERROR_TEXT_LENGTH];
    ...
    if (lex) {
        ...
        if (saved_text && saved_text[0]) {
            if (lex->saved_text.length <= 20) {
                snprintf(msg_with_context, sizeof(msg_with_context) - 1, "%s near '%s'",
                         msg_text, saved_text);
            ...
            if (lex->stream.state == STREAM_STATE_ERROR) {
                ...
                snprintf(msg_with_context, sizeof(msg_with_context) - 1, "%s near end of file",
                         msg_text);
                msg_with_context[JSON_ERROR_TEXT_LENGTH - 1] = '\0';
                result = msg_with_context;
            }
